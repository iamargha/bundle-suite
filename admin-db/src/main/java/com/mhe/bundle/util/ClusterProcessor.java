...
import com.mhe.bundle.common.OpenAPIConfiguration;
...
public class ClusterProcessor {
	...
	private void populateServerInfo() throws BundleException {
		logger.debug("****************Inside printClusterInfo Entry******************");
		List<ServerInfo> serverInfos = new ArrayList<ServerInfo>();
		ClusterInfo clusterInfo = null;
		String clusterName = null;
		...
		String[] ipPort = null;
		try {
			servers = OpenAPIConfiguration.getSystemValue("app.servers").split(",");
			//servers = Constants.servers.split(",");
			for (String server : servers) {
				//clusterName = (String) connection.getAttribute(cl, "Name");
				clusterName = "PQA_CLUSTER";
				ServerInfo serverInfo = new ServerInfo();
				//logger.debug("######################## \n cl : {}" , cl);
				clusterInfo = new ClusterInfo();
				clusterInfo.setClusterName(clusterName);
				...
				//address = InetAddress.getByName(listenAddress);
				//listenPort = (Integer) connection.getAttribute(ser, "ListenPort");
				server = server.replace(serverInfo.getProtocol(),"");
				ipPort = server.split(":");
				serverInfo.setServerIp(ipPort[0]);
				//serverInfo.setServerName(serverName);
				serverInfo.setServerPort(ipPort[1]);
				logger.debug("serverInfo :: {}" , serverInfo);
				serverInfos.add(serverInfo);
				//}
